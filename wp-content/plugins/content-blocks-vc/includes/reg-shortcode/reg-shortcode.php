<?php
if(!class_exists('my_contentblocks_bete_sc')){
	class my_contentblocks_bete_sc {
		...
		static function init(){
			...
			add_shortcode('cbb_layout', array(__CLASS__, 'handle_shortcode'));
			add_shortcode('cbb_layout_filter', array(__CLASS__, 'filter_container'));
			add_shortcode('cbb_layout_filter_items', array(__CLASS__, 'filter_items'));
			add_shortcode('cbb_layout_order_by', array(__CLASS__, 'order_by'));
			add_shortcode('cbb_layout_sort_order', array(__CLASS__, 'sort_order'));	
		...
		static function handle_shortcode($params, $contents=''){
			...
			$query_params['woo_query'] 			= (isset($params['woo_query'])&&trim($params['woo_query'])!=''&&is_numeric(trim($params['woo_query'])))?trim($params['woo_query']):'0';
			...
			//Build Options	
			$options = array();			
			$options['layout_style']			= (isset($params['layout_style'])&&trim($params['layout_style'])!=''&&is_numeric(trim($params['layout_style'])))?trim($params['layout_style']):'3';
			...
			$options['s_image_post_format']	= (isset($params['s_image_post_format'])&&trim($params['s_image_post_format'])!=''&&is_numeric(trim($params['s_image_post_format'])))?trim($params['s_image_post_format']):'0';
			$options['s_image_post_format_pos']	= (isset($params['s_image_post_format_pos'])&&trim($params['s_image_post_format_pos'])!='')?trim($params['s_image_post_format_pos']):'ul-bottom-right';
			//image options
			...
			$options['s_excerpt_rbtn']			= (isset($params['s_excerpt_rbtn'])&&trim($params['s_excerpt_rbtn'])!=''&&is_numeric(trim($params['s_excerpt_rbtn'])))?trim($params['s_excerpt_rbtn']):'0';
			...
			$options['before_author_text']		= (isset($params['before_author_text'])&&trim($params['before_author_text'])!='')?trim($params['before_author_text']):'';
			...
			$options['geodirectory_rating']		= (isset($params['geodirectory_rating'])&&trim($params['geodirectory_rating'])!=''&&is_numeric(trim($params['geodirectory_rating'])))?trim($params['geodirectory_rating']):'0';
			...
			if(function_exists('vc_shortcode_custom_css_class')){
				$options['css_class'] = apply_filters(VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, vc_shortcode_custom_css_class($css, ' '), 'cbb_layout', $params);
			}
			...
			return $inline_css.my_contentblocks_bete_html::html_builder($query_params, $filter, $order, $orderby, $sub_opt_query, $options, $contents);
		}
			...
			static function filter_container($params, $contents=''){
				...
				$display_type		= (isset($params['display_type'])&&trim($params['display_type'])!=''&&is_numeric(trim($params['display_type'])))?trim($params['display_type']):'2';
				$title 				= (isset($params['title'])&&trim($params['title'])!='')?trim($params['title']):'';
				$title_link			= (isset($params['title_link'])&&trim($params['title_link'])!='')?trim($params['title_link']):'';
				...
				ob_start();				
					...
					if($title!=''){
						...
						$after_title = '';
						if($title_link!=''){
							$before_title = '<a href="'.$title_link.'" title="'.esc_attr($title).'" class="ultimate-layouts-tab-link">';
							$after_title = '</a>';
						}
						$filter_title = '<div class="ul-smart-tab-title-wrap"><div class="ul-smart-tab-title">'.$before_title.$title.$after_title.'</div></div>';
					}
			...
			static function filter_items($params){
				...
				ob_start();					
					if($filter_items!=''){
						...
						$show_all_echo 		= 	($show_all_text!='')?$show_all_text:__('Show All', 'cb_blocks_layouts');
						$dd_title_text_echo	= 	($dd_title_text!='')?$dd_title_text:__('Filter', 'cb_blocks_layouts');
			...
			static function order_by($params){
                ...
                <div class="ultimate-layouts-filter-item ul-order-by-action <?php echo esc_attr($extra_class);?>">
                    <span class="order-by-default" data-order-by="none">
                        <span class="ul-text-order-by"><?php echo ($sbn_text!='')?esc_html($sbn_text):__('Sort By None', 'cb_blocks_layouts');?></span>
                        <i class="fa fa-angle-down ul-arrow-angle" aria-hidden="true"></i>
                    </span>                     
                    
                    <span class="order-by-dropdown">
                    	<?php if($sortb_date=='1'){?>
                    		<span class="order-by-dropdown-item" data-order-by="date"><?php echo __('Sort By Date', 'cb_blocks_layouts')?></span>
                        <?php }
						if($sortb_id=='1'){
						?>
                        	<span class="order-by-dropdown-item" data-order-by="ID"><?php echo __('Sort By ID', 'cb_blocks_layouts')?></span>
                        <?php }
						if($sortb_title=='1'){
						?>
                        	<span class="order-by-dropdown-item" data-order-by="title"><?php echo __('Sort By Title', 'cb_blocks_layouts')?></span>
                        <?php }
						if($sortb_comment=='1'){
						?>
                       	 <span class="order-by-dropdown-item" data-order-by="comment_count"><?php echo __('Sort By Comment', 'cb_blocks_layouts')?></span>
                        <?php }
						if($sortb_author=='1'){
						?>
                        	<span class="order-by-dropdown-item" data-order-by="author"><?php echo __('Sort By Author', 'cb_blocks_layouts')?></span>
                        <?php }
						if($sortb_price=='1'){
						?>
                        	<span class="order-by-dropdown-item" data-order-by="_price"><?php echo __('Sort By Price', 'cb_blocks_layouts')?></span>
                        <?php }
						if($sortb_sales=='1'){
						?>
                        	<span class="order-by-dropdown-item" data-order-by="total_sales"><?php echo __('Sort By Sales', 'cb_blocks_layouts')?></span>
                        <?php }
						if($sortb_rating=='1'){
						?>
                        	<span class="order-by-dropdown-item" data-order-by="_wc_average_rating"><?php echo __('Sort By Rating', 'cb_blocks_layouts')?></span>
                        <?php }?>
                    </span>                   
                </div>
                <?php
		...
		static function ajax(){
			...
			if(isset($query_params) || isset($filter) || isset($order) || isset($orderby) || isset($sub_opt_query) || isset($options)){
				...
				echo my_contentblocks_bete_html::html_builder($query_params, $filter, $order, $orderby, $sub_opt_query, $options, '');
			}else{
				echo '';
			}
			exit;
				...
				if($newQuery->have_posts()):
					...
					while($newQuery->have_posts()):$newQuery->the_post();
						...
						$post_metas_1			= my_ultimateLayouts_elements::ultimateLayouts_metas_1(
													...
													array($qv_s_metas_o_time, $qv_time_format), $qv_s_metas_o_comment, $qv_s_metas_o_like, false, $qv_custom_meta_o, '', $before_author_text, ''
												  );
		...
		static function front_enqueue_scripts(){			
			wp_enqueue_style('beeteam_front_fontawsome_css', 		CBB_BETE_PLUGIN_URL.'assets/front-end/fontawesome/css/font-awesome.min.css', array(), CBB_BETE_VER);
			wp_enqueue_style('beeteam_front_slick_css', 			CBB_BETE_PLUGIN_URL.'assets/front-end/slick/slick.css', array(), CBB_BETE_VER);
			wp_enqueue_style('beeteam_front_loadawsome_css', 		CBB_BETE_PLUGIN_URL.'assets/front-end/loaders.css', array(), CBB_BETE_VER);
			wp_enqueue_style('beeteam_front_priority_css', 			CBB_BETE_PLUGIN_URL.'assets/front-end/priority-navigation/priority-nav-core.css', array(), CBB_BETE_VER);		
			wp_enqueue_style('beeteam_front_hover_css', 			CBB_BETE_PLUGIN_URL.'assets/front-end/hover-css/hover.css', array(), CBB_BETE_VER);
			//wp_enqueue_style('beeteam_front_pagination_css', 		CBB_BETE_PLUGIN_URL.'assets/front-end/pagination/pagination.css', array(), CBB_BETE_VER);
			wp_enqueue_style('beeteam_front_malihu_css', 			CBB_BETE_PLUGIN_URL.'assets/front-end/malihuscroll/jquery.mCustomScrollbar.min.css', array(), CBB_BETE_VER);
			
			wp_enqueue_script('beeteam_front_lazysizes_js', 		CBB_BETE_PLUGIN_URL.'assets/front-end/lazysizes.js', array(), CBB_BETE_VER, false);	
		}
		
		//CSS Core
		static function core_enqueue_scripts(){		
			wp_enqueue_style('ul_bete_front_css', 					CBB_BETE_PLUGIN_URL.'assets/front-end/core.css', array(), CBB_BETE_VER);
			if(trim(get_option('ultimate_layouts_rtl_mode'))=='1'){
				wp_enqueue_style('ul_bete_front_rtl_css', 				CBB_BETE_PLUGIN_URL.'assets/front-end/rtl.css', array(), CBB_BETE_VER);
			}
		}
		/*CSS Library*/
		...
		static function register_scripts(){//Change in function	
			//languages
			load_plugin_textdomain('cb_blocks_layouts', false, basename(CBB_BETE_PLUGIN_PATH).'/languages');
		
			//CSS Library
			self::front_enqueue_scripts();
			
			//JS Library				
			wp_register_script('beeteam_front_slick_js', 			CBB_BETE_PLUGIN_URL.'assets/front-end/slick/slick.clones.min.js', array('jquery'), CBB_BETE_VER, true);
			wp_register_script('beeteam_front_priority', 			CBB_BETE_PLUGIN_URL.'assets/front-end/priority-navigation/priority-nav.min.js', array('jquery'), CBB_BETE_VER, true);
			wp_register_script('beeteam_front_malihu_js', 			CBB_BETE_PLUGIN_URL.'assets/front-end/malihuscroll/jquery.mCustomScrollbar.concat.min.js', array('jquery'), CBB_BETE_VER, true);
					
			//JS Core
			//wp_register_script('ul_bete_front_js', 					CBB_BETE_PLUGIN_URL.'assets/front-end/core.js', array('jquery'), CBB_BETE_VER, true);			
			wp_register_script('ul_bete_front_js', 						CBB_BETE_PLUGIN_URL.'assets/front-end/core-min.js', array('jquery'), CBB_BETE_VER, true);
		}	
		/*Register Script*/		
